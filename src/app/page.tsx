...
export default function HomePage() {
  ...
  const filteredEvents = (data.events || []).filter((ev) => {
    ...
    if (eventFilter === 'completed' || eventFilter === 'closed') return ev.status.toLowerCase() === 'completed' || ev.status.toLowerCase() === 'closed';
              ...
              {filteredEvents.length > 0 ? (
                ...
                >
                  ...
                  <div style={{ color: '#fff', fontSize: '1.05rem', fontWeight: 600, letterSpacing: '0.04em', marginBottom: 8 }}>
                    [ OPERATIONS NOTICE // SCHEDULE ]
                  </div>
                  <div style={{ color: 'var(--text-muted)', fontSize: '0.92rem', lineHeight: 1.6 }}>
                    Club events, CTFs, and hackathon schedules are currently being planned for 2026.
                  </div>
                  <div style={{ color: 'var(--accent-cyan)', fontSize: '0.78rem', marginTop: 12, opacity: 0.85 }}>
                    // UPCOMING OPERATIONS WILL BE BROADCAST HERE
                  </div>
                </div>
              )}
            </div>
      ...
      {v.team && (
        <section id="team" className="section" style={{ background: 'rgba(5, 8, 17, 0.4)' }}>
          <div className="container">
            ...
            {/* Student Core Leads */}
            {(data.team || []).some((m) => m.category === 'student') && (
              <>
                <div className="team-role-heading" style={{ marginTop: '3rem' }}>
                  <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
                    <path d="M17 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2" />
                    <circle cx="9" cy="7" r="4" />
                  </svg>
                  <span>Student Core Leads</span>
                </div>
